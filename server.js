...
const dbme = new Datastore({ filename: ".data/dbme.json", autoload: true });
...
const listBerita = [
  ...
  {
    title:
      "8 <em>Startup</em> Tahap Awal yang Bergabung dengan Program Akselerator Digitaraya",
    slug: "startup-peserta-akselerator-digitaraya"
  },
  {
    title:
      "Sejarah Perkembangan Aplikasi <em>Instant Messaging</em> di Seluruh Dunia",
...
app.get("/api/news/:slug", (req, res) => {
  dbme.find({ slug: req.params.slug }, function(err, docs) {
    res.send(docs);
  });
});
...
app.put("/api/news/:slug", (req, res) => {
  ...
  db.update({ slug: req.params.slug }, news, {}, function(err, numReplaced) {
    res.send("ok");
  });
});
...
app.delete("/api/berita/:slug", (req, res) => {
  dbme.remove({ slug: req.params.slug }, {}, function(err, numRemoved) {
    res.send("ok");
  });
});
